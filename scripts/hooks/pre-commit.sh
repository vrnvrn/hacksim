...
# extended regex with LC_ALL=C for byte-exact matching across GNU and BSD grep.
...
# Em dash or en dash in any added line.
if printf '%s' "$DIFF" | LC_ALL=C grep -qE "^\+.*(${em}|${en})"; then
  fail "em or en dash"
fi

# Lowercase rhetorical contrast as a whole word.
if printf '%s' "$DIFF" | LC_ALL=C grep -qiE "^\+.*(^|[^A-Za-z])${nj}([^A-Za-z]|\$)"; then
  fail "rhetorical contrast (lowercase)"
fi

# Titlecase rhetorical contrast as a whole word.
if printf '%s' "$DIFF" | LC_ALL=C grep -qE "^\+.*(^|[^A-Za-z])${no}([^A-Za-z]|\$)"; then
