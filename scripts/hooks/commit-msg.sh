...
# does not flag itself when staged. POSIX ERE for cross-platform grep.
...
if grep -qiE "${ca}:" "$MSG_FILE"; then
  fail "co-author trailer"
fi
if grep -qiE "${gw}" "$MSG_FILE"; then
  fail "generated-with attribution"
fi
if grep -qE "[${em}${en}]" "$MSG_FILE"; then
  fail "em or en dash"
fi
if grep -qiE "(^|[^A-Za-z])${nj}([^A-Za-z]|\$)" "$MSG_FILE"; then
  fail "rhetorical contrast (lowercase)"
fi
if grep -qE "(^|[^A-Za-z])${no}([^A-Za-z]|\$)" "$MSG_FILE"; then
